...
from .data import parse_excel, make_documents, load_and_split_file
...
class FaissStoreHandler:
    ...
    def add_documents(self, split_documents: list[Document]) -> None:
        ...
        for i, doc in enumerate(split_documents):
            if not doc.page_content.strip():
                print(f"⚠️ Пустой документ #{i}: {doc.metadata}")
    ...
    def add_and_save_raw_files(
        ...
        match mode:
            case "scenario":
                exit_nodes = parse_excel("data/scenario/docs/exit_points.xlsx")
            ...
            case "knowledge":
                ...
                docs = load_and_split_file(
                    path_to_file=path_to_file, source_name=source
                )
